...
//エラーが出ない
    ...
    else if(($_POST['prefecture_code'])==="empty"){
        $error['prefecture_code']='blank';
    }
    else if(!preg_match("/^[0-9]+$/", $_POST['prefecture_code'])){ //空文字ダメの半角数値
        $error['prefecture_code']='type';
    }else if(($_POST['prefecture_code'])>47 || ($_POST['prefecture_code'])<1){
    ...
    if(!preg_match("/^[0-9a-zA-Z]+$/", $_POST['prefix'])){//半角英数字、空文字NG
        $error['prefix']='type';
    }
    var_dump($error);
}
    ...
    <table class="join_table">
        ...
        <tr><th>担当者名</th> 
            <td>
                <input class="text_join" type="text"name="manager" 
                    value="<?php if (!empty($_POST['manager'])){echo h($_POST['manager']);}?>">
        ...
        <tr><th>電話番号</th> 
            <td>
                <input class="text_join" type="text" name="phone"
                    value="<?php if (!empty($_POST['phone'])){echo h($_POST['phone']);}?>">
        ...
        <tr><th rowspan="3">住所</th> 
            <td>郵便番号 <input class="text_join_address" type="text" name="postal_code"
                value="<?php if (!empty($_POST['postal_code'])){echo h($_POST['postal_code']);}?>">
            ...
            <tr><td>市区町村 <input class="text_join_address" type="text" name="address"
                value="<?php if (!empty($_POST['address'])){echo h($_POST['address']);}?>">
        ...
        <tr><th>プレフィックス</th> 
            <td>
                <input class="text_join" type="text" name="prefix"
                    value="<?php if (!empty($_POST['prefix'])){echo h($_POST['prefix']);}?>">
